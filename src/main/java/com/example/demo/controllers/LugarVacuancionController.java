...
public class LugarVacuancionController {
    ...
    public List<Lugar> findAll(){
        List lugares = new ArrayList<Lugar>();
        int cont = 0;
        try {
            ...
            for(int i = 0; i < results.size(); i++)
            {
                ...
                if(ind.getOntClass().getLocalName().toString().equals("Lugar_Vacunacion")){
                    cont = 0;
                    lugar = new Lugar();
                    lugar.setUri(ind.getURI());
                    ...
                    while ( it.hasNext()) {
                        ...
                        if (s.getObject().isLiteral()) {
                            if(cont == 0){
                                lugar.setNombre(s.getLiteral().getLexicalForm().toString());
                            }
