...
import com.example.demo.Models.Lote;
    ...
    public List<Lote> findAll(){
        ...
        try {
            ...
            for(int i = 0; i < results.size(); i++)
            {
                ...
                if(ind.getOntClass().getLocalName().toString().equals("Lote")){
                    cont = 0;
                    lote = new Lote();
                    lote.setUri(ind.getURI());
                    ...
                    while ( it.hasNext()) {
                        ...
                        if (s.getObject().isLiteral()) {
                            if(cont == 0){
                                lote.setNum_vacunas(parseInt(s.getLiteral().getLexicalForm()));
                            }
                            if(cont == 2){
                                lote.setID(s.getLiteral().getLexicalForm().toString());
                            }
                            if(cont == 1){
                                lote.setVac_disponibles(parseInt(s.getLiteral().getLexicalForm()));
                            }
